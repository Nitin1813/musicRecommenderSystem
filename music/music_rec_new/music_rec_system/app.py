...
def recommend(music_title):
    k = 5
    music_index = music[music['title'] == music_title].index[0]
    similarity_scores = similarity[music_index]
    k_nearest_indices = np.argsort(similarity_scores)[::-1][1:k+1]
    recommended_music_titles = music.iloc[k_nearest_indices]['title'].tolist()
    
    return recommended_music_titles


music_dict = pickle.load(open(r'C:\Users\Nitin\Desktop\music\music_rec.pkl', 'rb'))
music = pd.DataFrame(music_dict)

similarity = pickle.load(open(r'C:\Users\Nitin\Desktop\music\similarities.pkl', 'rb'))
